// The portfolio as it was before it moved into Supabase (edited at
// /invoices_login/portfolio). Kept for two jobs only:
//   1. the admin page's one-time "Import current website" button, which
//      uploads these photos from public/work/ into Supabase Storage;
//   2. a fallback the public site renders while Supabase has no projects yet
//      (or can't be reached at build time), so the site is never empty.
// Don't edit content here — use the admin page.
...
export const LEGACY_ABOUT_PHOTO = "/work/ggs-iceland.jpg";
...
export const LEGACY_PROJECTS: Project[] = ENTRIES.map((p) => ({ ...p, cover: p.cover ?? p.photos[0]?.src }));
